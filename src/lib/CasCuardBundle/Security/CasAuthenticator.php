...
namespace App\lib\CasCuardBundle\Security;

use App\lib\CasCuardBundle\Event\CasAuthenticationFailureEvent;
...
class CasAuthenticator extends AbstractAuthenticator {
    ...
    public function authenticate(Request $request): Passport 
    {
        ...
        \phpCAS::client(CAS_VERSION_2_0, $this->getParameter('host2'), $this->getParameter('port2'), is_null($this->getParameter('path2')) ? '' : $this->getParameter('path2'), true);

        if(is_bool($this->getParameter('ca2')) && $this->getParameter('ca2') == false) {
            \phpCAS::setNoCasServerValidation();
        } else {
            \phpCAS::setCasServerCACert($this->getParameter('ca2'));
        }

        if($this->getParameter('handleLogoutRequest2')) {
            if ($request->request->has('logoutRequest2')) {
                $this->checkHandleLogout($request);
            }
            $logoutRequest = $request->request->get('logoutRequest2');
        ...
        if ($this->getParameter('gateway2')) {
            
            // .. code de pierre pelisset (pour les applis existantes...)
            
            if($this->getParameter('force2')) {
    ...
    public function getParameter($key) {
        if(!array_key_exists($key, $this->config)) {
            throw new InvalidConfigurationException('l3_cas_cuard.' . $key . ' is not defined');
        }
